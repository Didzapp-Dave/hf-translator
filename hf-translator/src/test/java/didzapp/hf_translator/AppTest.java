...
public class AppTest {
	...
	void testAllLanguages() {
		...
		try {
			// 1. rightToLeftDetect
			final boolean rtl = Translator.isRightToLeft(l);
			...
			// 1. formatNumber
			final String formattedNum = Translator.formatNumber(l, Double.valueOf(100.100));
			...
			// 2. formatTimestamp
			final String formattedTs = Translator.formatTimestamp(l, Translator.quickTimestamp.timestamp());
			...
			final String formattedLdt = Translator.formatLocalDateTime(l, Translator.quickTimestamp.timestamp().toLocalDateTime());
			...
			final String formattedMy = Translator.formatTimestamp_MonthYear(l, Translator.quickTimestamp.timestamp());
			...
			// 5. formatDate
			final String formattedDate = Translator.formatDate(l, Translator.quickTimestamp.timestamp().toString());
			...
			// 6. formatTime
			final String formattedTime = Translator.formatTime(l, Translator.quickTimestamp.timestamp().toString());
			...
			// 7. formatCurrency
			final String formattedCurr = Translator.formatCurrency(l, "£100.10"); //$NON-NLS-1$
			...
			// 8. parseDate
			final Date parsedDate = Translator.parseDate(l, Translator.quickTimestamp.timestamp().toString());
			System.out.println("parseDate: " + parsedDate); //$NON-NLS-1$
			assertNotNull(parsedDate, "parseDate returned null for " + l); //$NON-NLS-1$
			// 9. parseTime
			final String timeStr = Translator.quickTimestamp.timestamp().toLocalDateTime().toLocalTime().toString();
			final Time parsedTime = Translator.parseTime(l, timeStr);
			...
			final int timeOfDay = Translator.determinTimeOfDay(l, ZoneId.of("Europe/London")); //$NON-NLS-1$
